...
export const api = {
  ...
  // HR
  hr: {
    // Legacy employees endpoints removed
    persons: {
      search: (query?: string, page: number = 1, limit: number = 20, effectiveDate?: string) => {
        const params = new URLSearchParams();
        if (query) params.append("q", query);
        params.append("page", page.toString());
        params.append("limit", limit.toString());
        if (effectiveDate) params.append("effectiveDate", effectiveDate);
        return fetch(`${API_BASE}/api/hr/persons?${params.toString()}`, { credentials: "include" }).then(r => r.json());
      },
      getTransactions: () => fetch(`${API_BASE}/api/hr/transactions`, { credentials: "include" }).then(r => r.json()),
      getAnalytics: () => fetch(`${API_BASE}/api/hr/analytics`, { credentials: "include" }).then(r => r.json()),
      get: (id: string) => fetch(`${API_BASE}/api/hr/persons/${id}`, { credentials: "include" }).then(r => r.json()),
      create: (data: any) => apiRequest("POST", `${API_BASE}/api/hr/persons`, data).then(r => r.json()),
      terminate: (id: string, data: any) => apiRequest("POST", `${API_BASE}/api/hr/persons/${id}/terminate`, data).then(r => r.json()),
      transfer: (id: string, data: any) => apiRequest("POST", `${API_BASE}/api/hr/persons/${id}/transfer`, data).then(r => r.json()),
    },
    documents: {
      list: (personId: string) => fetch(`${API_BASE}/api/hr/persons/${personId}/documents`, { credentials: "include" }).then(r => r.json()),
      create: (data: any) => apiRequest("POST", `${API_BASE}/api/hr/documents`, data).then(r => r.json()),
    },
    checklists: {
      listTemplates: () => fetch(`${API_BASE}/api/hr/checklists/templates`, { credentials: "include" }).then(r => r.json()),
      assign: (data: { personId: string, checklistId: string, tenantId?: string }) => apiRequest("POST", `${API_BASE}/api/hr/checklists/assign`, data).then(r => r.json()),
      listAllocations: (personId: string) => fetch(`${API_BASE}/api/hr/persons/${personId}/allocations`, { credentials: "include" }).then(r => r.json()),
      getTasks: (allocationId: string) => fetch(`${API_BASE}/api/hr/allocations/${allocationId}/tasks`, { credentials: "include" }).then(r => r.json()),
      updateTask: (taskId: string, status: string) => apiRequest("POST", `${API_BASE}/api/hr/tasks/${taskId}/status`, { status }).then(r => r.json()),
    },
    leaves: {
    ...
    structures: {
      locations: {
        list: () => fetch(`${API_BASE}/api/hr/structures/locations`, { credentials: "include" }).then(r => r.json()),
        create: (data: any) => apiRequest("POST", `${API_BASE}/api/hr/structures/locations`, data).then(r => r.json()),
      },
      organizations: {
        list: (classification?: string) => fetch(`${API_BASE}/api/hr/structures/organizations${classification ? `?classification=${classification}` : ""}`, { credentials: "include" }).then(r => r.json()),
        create: (data: any) => apiRequest("POST", `${API_BASE}/api/hr/structures/organizations`, data).then(r => r.json()),
      },
      jobs: {
        list: () => fetch(`${API_BASE}/api/hr/structures/jobs`, { credentials: "include" }).then(r => r.json()),
        create: (data: any) => apiRequest("POST", `${API_BASE}/api/hr/structures/jobs`, data).then(r => r.json()),
      },
      grades: {
        list: () => fetch(`${API_BASE}/api/hr/structures/grades`, { credentials: "include" }).then(r => r.json()),
        create: (data: any) => apiRequest("POST", `${API_BASE}/api/hr/structures/grades`, data).then(r => r.json()),
      },
      positions: {
        list: () => fetch(`${API_BASE}/api/hr/structures/positions`, { credentials: "include" }).then(r => r.json()),
        create: (data: any) => apiRequest("POST", `${API_BASE}/api/hr/structures/positions`, data).then(r => r.json()),
      },
    },
    hdl: {
      importWorkers: (csv: string) => apiRequest("POST", `${API_BASE}/api/hr/hdl/workers`, { csv }).then(r => r.json()),
      getHistory: () => fetch(`${API_BASE}/api/hr/hdl/history`, { credentials: "include" }).then(r => r.json()),
    },
  },
