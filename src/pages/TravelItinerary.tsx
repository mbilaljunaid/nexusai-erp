...
export default function TravelItinerary() {
    const { data: itineraries = [], isLoading } = useQuery<any[]>({
        queryKey: ["/api/hospitality-itinerary"],
        queryFn: () => fetch("/api/hospitality-itinerary").then(r => r.json()).catch(() => []),
    });
    const confirmed = itineraries.filter((i: any) => i.status === "confirmed").length;
    return (
        <StandardPage
            title="Travel Itinerary & Ticketing"
            description="Itineraries, flights, transport, packages"
            className="space-y-6"
        >
            <div className="grid grid-cols-4 gap-3">
                <Card className="p-3"><CardContent className="pt-0"><p className="text-xs text-muted-foreground">Itineraries</p><p className="text-2xl font-bold">{itineraries.length}</p></CardContent></Card>
                <Card className="p-3"><CardContent className="pt-0"><p className="text-xs text-muted-foreground">Confirmed</p><p className="text-2xl font-bold text-green-600">{confirmed}</p></CardContent></Card>
