...
export default function CommitmentDashboard() {
    ...
    const varColumns: SpreadsheetColumn<VarRow>[] = [
        ...
        { id: "period", header: "Period", width: "100px", cell: (v) => <span className="text-gray-500 whitespace-nowrap">{formatNumber(fmtDate(v.period_start)}</span> },
        ...
        { id: "planCost", header: "Plan Cost", width: "120px", cell: (v) => <span className="font-mono">{formatNumber(fmt(v.planned_cost)}</span> },
        { id: "actCost", header: "Act Cost", width: "120px", cell: (v) => <span className="font-mono">{formatNumber(fmt(v.actual_cost)}</span> },
        { id: "delCost", header: "Δ Cost", width: "120px", cell: (v) => <span className={cn(`font-mono ${v.cost_variance < 0 ? 'text-red-600' : 'text-emerald-600'}`)}>{v.cost_variance > 0 ? '+' : ''}{formatNumber(fmt(v.cost_variance)}</span> },
        {
            id: "varPct", header: "Var %", width: "100px", cell: (v) => {
                const overBudget = Number(v.variance_pct) > 0;
                return <span className={cn(`font-bold ${overBudget ? 'text-red-600' : 'text-emerald-600'}`)}>{overBudget ? '+' : ''}{formatNumber(Number(v.variance_pct).toFixed(1)}%</span>;
            }
        },
    ];
                                ...
                                return (
                                    <div key={a.id} className={cn(`rounded-xl p-2.5 px-3.5 flex justify-between items-center border border-l-[4px] ${cfg.bg} ${cfg.border} ${cfg.borderLeft}`)}>
                                        <div>
                                            ...
                                            {a.budget_amount && <div className="text-[10px] text-gray-500 mt-0.5">Budget: {formatNumber(fmt(a.budget_amount)} · Actual: {formatNumber(fmt(a.actual_amount)} · Variance: {formatNumber(Number(a.variance_pct).toFixed(1)}%</div>}
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <span className="text-[10px] text-gray-400">{formatNumber(fmtDate(a.created_at)}</span>
                                            {!a.is_acknowledged && (
