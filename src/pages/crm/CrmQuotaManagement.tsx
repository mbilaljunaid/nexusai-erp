...
export default function CrmQuotaManagement() {
    ...
    return (
        ...
        >
            ...
            {/* Rep List */}
            <Card className="border-none shadow-sm ring-1 ring-slate-200 rounded-2xl overflow-hidden bg-card">
                <CardHeader className="border-b border-slate-50 px-8 py-6">
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle className="text-xl font-bold tracking-tight text-foreground">Representative Targets</CardTitle>
                            <CardDescription className="text-muted-foreground/70 font-medium mt-1">Detailed breakdown of attainment for {selectedPeriod}</CardDescription>
                        </div>
                        <div className="flex gap-2">
                            <Button variant="outline" size="sm" className="rounded-lg h-8 border-border text-muted-foreground font-bold text-[11px] uppercase tracking-wider">
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-muted/50/50 hover:bg-muted/50/50 border-border">
                                <TableHead className="w-72 h-12 uppercase text-[10px] font-black tracking-widest text-muted-foreground/70 px-8">Sales Representative</TableHead>
                                <TableHead className="h-12 uppercase text-[10px] font-black tracking-widest text-muted-foreground/70">Target Type</TableHead>
                                <TableHead className="h-12 uppercase text-[10px] font-black tracking-widest text-muted-foreground/70">Quota Amount</TableHead>
                                <TableHead className="h-12 uppercase text-[10px] font-black tracking-widest text-muted-foreground/70">Actual Revenue</TableHead>
                                <TableHead className="h-12 uppercase text-[10px] font-black tracking-widest text-muted-foreground/70 px-8">Attainment</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {users.map((user: any) => {
                                ...
                                return (
                                    <TableRow key={user.id} className="hover:bg-muted/50/30 transition-colors border-slate-50 group">
                                        <TableCell className="px-8 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div className="font-bold text-sm text-foreground dark:text-slate-200 group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{user.name}</div>
                                                    <div className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-wider">T-1 Territory A</div>
                                                </div>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-bold text-muted-foreground uppercase tracking-tight">Revenue</span>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <div className="font-bold text-sm text-muted-foreground tracking-tight">${formatNumber(Number(perf.quota))}</div>
                                        </TableCell>
                                        <TableCell>
                                            <div className="font-bold text-sm text-foreground dark:text-slate-200 tracking-tight">${formatNumber(Number(perf.actual))}</div>
                                        </TableCell>
