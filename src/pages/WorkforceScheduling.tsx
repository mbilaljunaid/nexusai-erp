...
export default function WorkforceScheduling() {
  ...
  return (
    <StandardPage
      ...
      description="Employee scheduling, shift management, and labor optimization"
    >
      ...
      <Card data-testid="card-new-schedule">
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-5 gap-2">
            ...
            <Button disabled={createMutation.isPending || !newSchedule.employeeId} size="sm" data-testid="button-create" onClick={() => createMutation.mutate(newSchedule)}>
          ...
          {isLoading ? <TableSkeleton rows={4} /> : schedules.length === 0 ? <p className="text-muted-foreground text-center py-4">No schedules</p> : schedules.map((s: any) => (
            <div key={s.id} className="p-2 border rounded text-sm hover-elevate flex items-center justify-between" data-testid={`schedule-${s.id}`}>
              ...
              <div className="flex gap-2 items-center">
                ...
                <Button size="icon" variant="ghost" data-testid={`button-delete-${s.id}`} className="h-7 w-7" onClick={() => deleteMutation.mutate(s.id)} aria-label="Delete">
