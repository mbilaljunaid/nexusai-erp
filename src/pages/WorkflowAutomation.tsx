...
export default function WorkflowAutomation() {
  const { data: workflows = [], isLoading } = useQuery<any[]>({
    queryKey: ["/api/gov-workflows"],
    queryFn: () => fetch("/api/gov-workflows").then(r => r.json()).catch(() => []),
  });
  ...
  return (
    <StandardPage
      ...
      description="Approvals, task assignment, SLA, escalation"
    >
      <div className="grid grid-cols-4 gap-3">
        ...
        <Card className="p-3">
          <CardContent className="pt-0">
            ...
            <p className="text-2xl font-bold text-blue-600">{workflows.filter((w: any) => w.status === "completed").length}</p>
          </CardContent>
        </Card>
        <Card className="p-3">
          <CardContent className="pt-0">
            <p className="text-xs text-muted-foreground">Active %</p>
            <p className="text-2xl font-bold">{workflows.length > 0 ? ((active / workflows.length) * 100).toFixed(0) : 0}%</p>
          </CardContent>
        </Card>
      </div>
      <Card>
          ...
          {isLoading ? <TableSkeleton rows={4} /> : workflows.length === 0 ? <p className="text-muted-foreground text-center py-4">No workflows</p> : workflows.slice(0, 10).map((w: any) => (
            <div key={w.id} className="p-2 border rounded text-sm hover-elevate flex items-center justify-between" data-testid={`wf-${w.id}`}>
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">{w.stepId}</p>
              </div>
