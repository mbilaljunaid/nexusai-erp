      ...
      <Card data-testid="card-new-order">
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-5 gap-2">
            ...
            <Input placeholder="Table ID" value={newOrder.tableId} onChange={(e) => setNewOrder({ ...newOrder, tableId: e.target.value })} data-testid="input-tid" className="text-sm" />
            <Input placeholder="Items" type="number" value={newOrder.items} onChange={(e) => setNewOrder({ ...newOrder, items: e.target.value })} data-testid="input-items" className="text-sm" />
          ...
          {isLoading ? <TableSkeleton rows={4} /> : orders.length === 0 ? <p className="text-muted-foreground text-center py-4">No orders</p> : orders.map((o: any) => (
            <div key={o.id} className="p-2 border rounded text-sm hover-elevate flex items-center justify-between" data-testid={`order-${o.id}`}>
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">Table: {o.tableId} • {o.items} items • ${o.total}</p>
              </div>
              <div className="flex gap-2 items-center">
                <Badge variant={o.status === "served" ? "default" : "secondary"} className="text-xs">{o.status}</Badge>
                <Button size="icon" variant="ghost" onClick={() => deleteMutation.mutate(o.id)} data-testid={`button-delete-${o.id}`} className="h-7 w-7" aria-label="Delete">
