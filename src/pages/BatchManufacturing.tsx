...
import { Factory, Plus, Trash2, Save, Loader2 } from "lucide-react";
...
export default function BatchManufacturing() {
    ...
    mutationFn: async (updatedBatches: any[]) => {
      for (const batch of updatedBatches) {
        if (!batch.id || String(batch.id).startsWith('temp-')) {
          await fetch("/api/fb-batches", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...batch, id: undefined }) });
        } else {
          await apiRequest("PATCH", `/api/fb-batches/${batch.id}`, batch).catch(() => { });
        }
      }

      const deletedIds = batches.filter((c: any) => !updatedBatches.find((uc) => uc.id === c.id)).map((c: any) => c.id);
      for (const id of deletedIds) {
        await fetch(`/api/fb-batches/${id}`, { method: "DELETE" });
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/fb-batches"] });
      toast({ title: "Batches saved successfully" });
    },
  });
  ...
  return (
    <StandardPage
      title="Batch Manufacturing & Kitchen Execution"
      description="Production orders, scaled sheets, in-process QC, yield tracking, and genealogy"
    >
      <div className="space-y-6">
        ...
        <Card className="mt-6 border-none shadow-lg">
          <CardHeader className="bg-muted/30 pb-4 border-b flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-xl">Batch Manufacturing Grid</CardTitle>
              <CardDescription>Inline editable spreadsheet for production batches</CardDescription>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setLocalBatches([...localBatches, { id: `temp-${Date.now()}`, batchId: '', recipeId: '', quantity: '100', status: 'planned' }])}>
          ...
          <CardContent className="p-0">
            {isLoading ? (
              <div className="p-8 flex items-center justify-center text-muted-foreground"><Loader2 className="animate-spin h-6 w-6 mr-2" /> Loading batches...</div>
            ) : (
              <InteractiveSpreadsheet
                ...
                columns={[
                  ...
                  },
                  {
                    ...
                    width: "200px",
                    cell: (row, index, updateRow) => (
                      <Input className="h-9 w-full border-0 focus-visible:ring-0 bg-transparent" placeholder="Recipe #" value={row.recipeId || ''} onChange={(e) => updateRow("recipeId", e.target.value)} />
                    )
                  },
                  {
                    id: "quantity",
