...
export default function SupplierQualification() {
    const { data: suppliers = [], isLoading } = useQuery<any[]>({
        queryKey: ["/api/pharma-suppliers"],
        queryFn: () => fetch("/api/pharma-suppliers").then(r => r.json()),
    });
    const qualified = suppliers.filter((s: any) => s.status === "qualified").length;
    const audited = suppliers.filter((s: any) => s.auditStatus === "passed").length;
    return (
        <StandardPage
            title="Supplier Qualification & Management"
            description="CMO, contract labs, audits, certificates (COA, GMP), risk classification, performance tracking"
            className="space-y-6"
        >
            <div className="grid grid-cols-4 gap-3">
                <Card className="p-3"><CardContent className="pt-0"><p className="text-xs text-muted-foreground">Total Suppliers</p><p className="text-2xl font-bold">{suppliers.length}</p></CardContent></Card>
                <Card className="p-3"><CardContent className="pt-0"><p className="text-xs text-muted-foreground">Qualified</p><p className="text-2xl font-bold text-green-600">{qualified}</p></CardContent></Card>
