...
export default function CampaignsDashboard() {
  ...
  return (
    ...
    >
      ...
      <Card><CardHeader><CardTitle>Campaigns</CardTitle></CardHeader><CardContent><div className="space-y-2">{isLoading ? <TableSkeleton rows={4} /> : campaigns.length === 0 ? <p className="text-muted-foreground text-center py-4">No campaigns</p> : campaigns.map((c: any) => (<div key={c.id} className="flex justify-between items-center p-3 border rounded hover-elevate" data-testid={`campaign-${c.id}`}><div><p className="font-semibold">{c.name}</p><p className="text-sm text-muted-foreground">Budget: ${c.budget} • Type: {c.type}</p></div><div className="flex gap-2 items-center"><Badge variant={c.status === "active" ? "default" : "secondary"}>{c.status}</Badge><Button size="icon" variant="ghost" onClick={() => deleteMutation.mutate(c.id)} data-testid={`button-delete-${c.id}`} aria-label="Delete"><Trash2 className="w-4 h-4" /></Button></div></div>))}</div></CardContent></Card>
    </StandardPage>
  );
}
