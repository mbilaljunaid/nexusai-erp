...
export default function DispatchConsole() {
    ...
    const { data: unassignedWOsResponse, isLoading: loadingWOs } = useQuery({
        queryKey: ["/api/maintenance/work-orders", "unassigned", page],
        queryFn: () => fetch(`/api/maintenance/work-orders?status=submitted&assignedToId=null&page=${page}&limit=${limit}`).then(r => r.json()),
    });
                        ...
                        <CardHeader>
                            <CardTitle className="flex justify-between">
                                ...
                                <div className="flex gap-2 items-center">
                                    <Badge variant="secondary">{unassignedWOsResponse?.total || 0} Pending</Badge>
                                </div>
                            </CardTitle>
                        </CardHeader>
