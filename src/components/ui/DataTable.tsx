...
export function DataTable<T extends Record<string, any>>({
  ...
  const totalPages = manualPagination ? (pageCount || 1) : Math.ceil(sortedData.length / pageSize);
    ...
    <div className="space-y-4" data-testid="data-table">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <span>
            {manualPagination
              ? `Showing ${(currentPage - 1) * pageSize + 1} to ${Math.min(currentPage * pageSize, rowCount || 0)} of ${rowCount || 0} results`
              : `Showing ${sortedData.length === 0 ? 0 : (currentPage - 1) * pageSize + 1} to ${Math.min(currentPage * pageSize, sortedData.length)} of ${sortedData.length} results`
            }
          </span>
        </div>
