...
export function AdrFlow({ ruleType, sourceType, constantValue, sourceAttribute, mappingSetName }: AdrFlowProps) {
    return (
        <div className="flex items-center justify-center p-8 bg-slate-50/50 rounded-xl border border-dashed border-slate-200 min-h-[300px] w-full relative overflow-hidden">
...
function FlowNode({ title, icon: Icon, children, color, delay }: any) {
    ...
    return (
        ...
        >
            <div className={`w-12 h-12 rounded-full border-2 flex items-center justify-center mb-4 shadow-sm z-20 bg-white ${colorStyles[color].replace('bg-', 'text-')}`}>
                <Icon className={`w-5 h-5`} />
            </div>

            <Card className="w-48 shadow-lg border-t-4 border-t-transparent hover:border-t-primary transition-all">
                <div className={`px-3 py-2 border-b flex items-center justify-between bg-slate-50/50 rounded-t-lg`}>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{title}</span>
                    <div className={`w-2 h-2 rounded-full ${colorStyles[color].split(' ')[2].replace('text-', 'bg-')}`}></div>
                </div>
