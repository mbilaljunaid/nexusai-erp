#!/usr/bin/env node
/**
 * Niche Verticals Migration Runner - FIXED
 * Runs prerequisites first, then all 12 niche vertical platform migrations
 */
...
async function main() {
    console.log('🚀 Niche Verticals Migration Runner - FIXED\n');
    console.log('Database:', process.env.DATABASE_URL?.split('@')[1] || 'Unknown');
    console.log('='.repeat(70));

    const migrationsDir = path.join(__dirname, 'supabase', 'migrations');

    // All niche vertical migrations in order - INCLUDING PREREQUISITES
    const migrations = [
        // STEP 0: Prerequisites (MUST RUN FIRST)
    ...
    console.log(`\n📋 Migration Plan (${migrations.length} migrations covering 30 modules):\n`);
    ...
    if (successful.length > 0) {
        ...
        try {
            ...
            // Check key tables from each phase
            const keyTables = [
    ...
    if (failed.length === 0) {
        ...
        console.log('   • Geography/GIS support');
