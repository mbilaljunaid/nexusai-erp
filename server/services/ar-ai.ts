...
export class ArAiService {
    ...
    async generateCollectionEmail(invoice: any, customer: any): Promise<string> {
        try {
            ...
            - Outstanding Amount: ${invoice.currency} ${invoice.totalAmount}
            - Reference Date: ${today.toLocaleDateString()}

            Tone Guidelines:
            - ${daysOverdue < 30 ? "Courteous reminder. Assume it might have been an oversight." : daysOverdue < 60 ? "Firm follow-up. Express concern and request immediate payment." : "Urgent demand. Mention potential impact on credit terms or service."}
