    ...
    async createAsset(data: any) {
        ...
        try {
            ...
            await slaEngine.createAccounting({
                ...
                currencyCode: "USD",
                eventDate: assetBook.datePlacedInService,
                glDate: assetBook.datePlacedInService,
    ...
    async approveRetirement(retirementId: string, userId: string) {
        ...
        try {
            ...
            await slaEngine.createAccounting({
                ...
                currencyCode: "USD",
                eventDate: retirement.retirementDate,
                glDate: retirement.retirementDate,
                ledgerId: book?.ledgerId || "PRIMARY",
                sourceData: {
                    gainLoss: Number(retirement.gainLossAmount),
                    proceeds: Number(retirement.proceedsOfSale),
                    costOfRemoval: Number(retirement.costOfRemoval),
                    netBookValue: Number(retirement.netBookValueRetired)
                }
            });
        } catch (e) {
