...
export class ConstructionService {
    ...
    private async postPayAppToGL(payAppId: string, userId: string) {
        ...
        try {
            await slaEngine.createAccounting({
                ...
                eventTypeId: "CONST_PROGRESS_BILLING",
                entityId: app.id,
                ...
                glDate: new Date(),
                currencyCode: "USD",
                amount: amount, // totalCompleted
                description: `Construction Pay App: ${app.applicationNumber}`,
                sourceData: {
                    applicationNumber: app.applicationNumber,
                    totalCompleted: amount,
                    retentionAmount: retention,
                    currentPaymentDue: Number(app.currentPaymentDue || 0),
                    ...app
                }
            });
    ...
    async getLaborLines(dailyLogId: string) {
        return await db.select().from(constructionDailyLabor).where(eq(constructionDailyLabor.dailyLogId, dailyLogId));
    }

    async addEquipmentLines(dailyLogId: string, lines: any[]) {
        const inserts = lines.map(l => ({ ...l, dailyLogId }));
        return await db.insert(constructionDailyEquipment).values(inserts).returning();
    }

    async getEquipmentLines(dailyLogId: string) {
        return await db.select().from(constructionDailyEquipment).where(eq(constructionDailyEquipment.dailyLogId, dailyLogId));
    }
