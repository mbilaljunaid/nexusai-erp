...
export class FinanceService {
    ...
    async postJournal(journalId: string, userId: string = "SYSTEM", context?: { ipAddress?: string, sessionId?: string }) {
        ...
        // 1.5 Approval Interception
        // If status is "Draft" or "Unposted", check if it needs approval first.
        if (journal.approvalStatus === "Pending") {
            throw new Error("Journal is pending approval. Cannot post.");
        }
        if (journal.approvalStatus === "Rejected") {
            throw new Error("Journal was rejected. Please correct and resubmit.");
        }

        if (journal.approvalStatus === "Required") {
            // It shouldn't be "Required" if it hasn't been submitted, but if it is, block it.
            // Or, we could auto-submit here? 
            // Let's enforce explicit submission via submitJournalForApproval or UI flow.
            throw new Error("Journal requires approval. Please submit for approval first.");
        }

        // If "Not Required" or "Approved", proceed.
    ...
    /**
     * Determines if a journal requires approval based on rules.
     */
    async evaluateApprovalRule(journal: GlJournal, totalDebit: number): Promise<string> {
        // 1. Fetch active rules for this ledger
        const rules = await db.select().from(glApprovalRules).where(
            and(
        ...
        if (rules.length === 0) return "Not Required";

        // 2. Check each rule
        // Priority: If ANY rule matches, it's Required. 
        // (Real world: might select specific approver chain, here just boolean "Required")
        for (const rule of rules) {
            // Check Amount
            const amount = totalDebit;
            if (rule.minAmount && amount < parseFloat(rule.minAmount)) continue;
            if (rule.maxAmount && amount > parseFloat(rule.maxAmount)) continue;

            // Check Source (e.g., Manual vs Spreadsheet)
            if (rule.source && rule.source !== journal.source) continue;

            // Check Category (e.g., Accrual vs Adjustment) - Not stored on Header currently, maybe inferred?
            // skipping category for now.
    ...
    async explainVariance(periodId: string, benchmarkPeriodId: string, ledgerId: string = "PRIMARY") {
        ...
        const currentMap = new Map(currentTB.rows.map(i => [i.code, i]));
        const variances: any[] = [];

        // 2. Compare
        for (const base of benchmarkTB.rows) {
            const curr = currentMap.get(base.code);
            // Fix: Use netBalance instead of netActivity
            // Ensure types are safe by casting or using optional chaining if netBalance is guaranteed
            const currentNet = curr ? Number((curr as any).netBalance || 0) : 0;
            const baseNet = Number((base as any).netBalance || 0);
        ...
        for (const j of postedJournals) {
            ...
            if (filtered.length > 0) {
                allLines.push(...filtered.map(l => ({
                    ...
                    amount: Number((j as any).totalDebit || 0), // totalDebit usually on batch, or derived
                    status: j.status,
                    date: j.createdAt
                })));
            }
    ...
    async createGlLegalEntity(data: { name: string; organizationId: string; registrationNumber?: string; ledgerId: string }) {
        // Use storage if possible or direct DB
    ...
    async updateGlLegalEntity(id: string, data: Partial<typeof glLegalEntities.$inferInsert>) {
