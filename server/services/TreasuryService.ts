...
export class TreasuryService {
  ...
  async generateISO20022(batchId: number): Promise<string> {
    ...
    // Fetch Internal Bank Account for Disbursement via Batch bankAccountId
    let internalBank;
    if (batch.bankAccountId) {
      [internalBank] = await db.select()
        .from(cashBankAccounts)
        .where(eq(cashBankAccounts.id, batch.bankAccountId))
        .limit(1);
    }

    if (!internalBank) {
      // Fallback: match currency if no specific bankAccountId on batch
    ...
    if (!internalBank) {
      throw new Error(`No active disbursement bank account found for currency ${payments[0].currencyCode}`);
    }

    const debtorIban = internalBank.accountNumber; // Using Account Number as IBAN proxy if IBAN col missing, or generic
    const debtorBic = internalBank.swiftCode;
    const debtorName = internalBank.bankName;

    if (!debtorIban || !debtorBic) {
      throw new Error(`Bank account ${internalBank.bankName} is missing SWIFT/IBAN details for ISO20022 transmission.`);
    }
