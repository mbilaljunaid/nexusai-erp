...
export async function registerRoutes(
  ...
  app.get("/api/dashboard/activities", async (req, res) => {
    try {
      ...
      const activities = [
        { id: "1", type: "ai", user: { name: "AI Assistant", initials: "AI" }, action: "identified high-value lead", target: "Sarah Johnson (Score: 87)", timestamp: "2 min ago" },
        { id: "2", type: "call", user: { name: "Alex Chen", initials: "AC" }, action: "logged a call with", target: "TechCorp Inc.", timestamp: "15 min ago" },
        { id: "3", type: "task", user: { name: "Maria Garcia", initials: "MG" }, action: "completed task", target: "Send Q4 proposal", timestamp: "1 hour ago" },
        { id: "4", type: "email", user: { name: "John Doe", initials: "JD" }, action: "sent email to", target: "12 leads in pipeline", timestamp: "2 hours ago" },
        { id: "5", type: "lead", user: { name: "AI Assistant", initials: "AI" }, action: "auto-created lead from", target: "Website form submission", timestamp: "3 hours ago" },
      ];
      res.json(activities);
    } catch (error) {
      res.json([]);
  ...
  app.get("/api/dashboard/kanban", async (req, res) => {
    try {
      // Fetch tasks from form data
      const taskRecords = await db.query.formData.findMany({
        where: (formData) => eq(formData.formId, "tasks"),
      });
      const tasks = taskRecords.map((r: any) => ({ id: r.id, ...r.data }));
      
      const columns = [
        {
          id: "todo",
          title: "To Do",
          tasks: tasks.filter((t: any) => t.status === "todo" || t.status === "pending").slice(0, 5).map((t: any) => ({
            id: t.id,
            title: t.title,
            priority: t.priority || "medium",
            assignee: t.assignee ? { name: t.assignee, initials: t.assignee.substring(0, 2).toUpperCase() } : undefined,
            dueDate: t.dueDate,
            tags: t.tags || [],
          })),
        },
        {
          id: "in_progress",
          title: "In Progress",
          tasks: tasks.filter((t: any) => t.status === "in_progress" || t.status === "active").slice(0, 5).map((t: any) => ({
            id: t.id,
            title: t.title,
            priority: t.priority || "medium",
            assignee: t.assignee ? { name: t.assignee, initials: t.assignee.substring(0, 2).toUpperCase() } : undefined,
        ...
        },
        {
          id: "review",
          title: "Review",
          tasks: tasks.filter((t: any) => t.status === "review").slice(0, 5).map((t: any) => ({
            id: t.id,
            title: t.title,
            priority: t.priority || "medium",
            assignee: t.assignee ? { name: t.assignee, initials: t.assignee.substring(0, 2).toUpperCase() } : undefined,
        ...
        },
        {
          id: "done",
          title: "Done",
          tasks: tasks.filter((t: any) => t.status === "done" || t.status === "completed").slice(0, 5).map((t: any) => ({
            id: t.id,
            title: t.title,
            priority: t.priority || "medium",
            assignee: t.assignee ? { name: t.assignee, initials: t.assignee.substring(0, 2).toUpperCase() } : undefined,
  ...
  app.get("/api/dashboard/analytics", async (req, res) => {
    try {
      ...
      // Return analytics data based on title/type
      const monthlyData = [
