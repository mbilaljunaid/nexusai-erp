...
export class MaintenanceService {
    ...
    async listWorkOrders(limit: number = 50, offset: number = 0, filters?: { status?: string, assignedToId?: string | null, organizationId?: string }) {
        ...
        if (filters?.organizationId) {
            // whereConditions.push(eq(maintWorkOrders.organizationId, filters.organizationId)); 
            // Commented out until schema supports it, but signature is ready.
        }

        const data = await db.query.maintWorkOrders.findMany({
        ...
        // (In a real high-volume scenario, we might estimate this or cache it)
        const totalRes = await db.select({ count: sql<number>`count(*)` })
            .from(maintWorkOrders)
            .where(whereConditions.length > 0 ? and(...whereConditions) : undefined);
     ...
     */
    async listOperationalAssets(filters?: { accountId?: string }) {
        const query = db.select({
            ...
            accountId: maintAssetsExtension.accountId,
            status: faAssets.status,
            model: faAssets.model,
            serialNumber: faAssets.serialNumber
        })
            .from(faAssets)
            .leftJoin(maintAssetsExtension, eq(faAssets.id, maintAssetsExtension.assetId));

        if (filters?.accountId) {
            query.where(eq(maintAssetsExtension.accountId, filters.accountId));
        }
