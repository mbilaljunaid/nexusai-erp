...
export class WmsTaskService {
    ...
    async completeTask(taskId: string, userId: string, actualQuantity: number, toLocatorId?: string) {
        return await db.transaction(async (tx) => {
            ...
            const [invTx] = await tx.insert(inventoryTransactions).values({
                itemId: task.itemId,
                transactionType: task.taskType,
                quantity: actualQuantity.toString(),
                transactionDate: new Date(),
                reference: task.taskNumber,
                sourceDocumentType: task.sourceDocType,
                sourceDocumentId: task.sourceDocId,
            }).returning();

            // 4. SLA Integration (PO Receipt / Adjustments)
            if (task.taskType === 'RECEIVE') {
                // Determine Cost (Mock for now, or fetch from PO)
                const unitCost = 50.00;
                const amount = actualQuantity * unitCost;

                await slaEngine.createAccounting({
                    eventClassId: "PO_RECEIPT",
                    eventTypeId: "PO_RECEIPT_STD",
                    entityId: invTx.id,
                    entityTable: "inv_material_transactions",
                    ledgerId: "PRIMARY",
                    eventDate: new Date(),
                    glDate: new Date(),
                    currencyCode: "USD",
                    amount: amount,
                    description: `PO Receipt: ${task.itemId}`,
                    sourceData: {
                        poId: task.sourceDocId,
                        itemId: task.itemId,
            ...
            // 5. Update Source Document (Order Line) if applicable
            ...
            // 6. SLA Integration (Inventory Adjustment - Count)
            if (task.taskType === 'COUNT') {
                const planned = Number(task.quantityPlanned);
                const actual = Number(actualQuantity);
                const diff = actual - planned;

                if (diff > 0) { // GAIN Scenario (Found Item)
                    const unitCost = 50.00; // Mock
                    const amount = diff * unitCost;

                    await slaEngine.createAccounting({
                        eventClassId: "INV_ADJUSTMENT", // Matches seed
                        eventTypeId: "INV_ADJUSTMENT_MISC", // Matches seed
                        entityId: invTx.id,
                        entityTable: "inv_material_transactions",
                        ledgerId: "PRIMARY",
                        eventDate: new Date(),
                        ...
                        currencyCode: "USD",
                        amount: amount,
                        description: `Inv Gain: ${task.itemId}`,
                        sourceData: {
                            taskId: task.id,
                            itemId: task.itemId,
                            diff: diff,
