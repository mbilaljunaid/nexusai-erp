...
export class ManufacturingService {
    ...
    async createWorkOrder(data: InsertProductionOrder) {
        // PJM Validation
        if (data.projectId) {
            // We can optionally validate if project exists here, but usually assume UI/API passed valid ID.
            // Ideally: await ppmService.validateTask(data.projectId, data.taskId);
        }
    ...
    async recordTransaction(data: InsertProductionTransaction) {
        const trx = await db.transaction(async (tx) => {
            const [trxResult] = await tx.insert(productionTransactions).values(data).returning();
        ...
        // Trigger Costing & Integration (Outside transaction to ensure visibility)
        // 1. WIP Accounting (GL)
        await manufacturingCostingService.processWipTransaction(
            trx.productionOrderId,
            trx.transactionType as any,
            Number(trx.quantity)
        );

        // 2. Project Costing (PPM)
        await manufacturingCostingService.processProjectCosting(trx.id);
