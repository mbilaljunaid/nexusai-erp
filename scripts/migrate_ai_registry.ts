...
const { PERMISSIONS } = require("../shared/schema/roles");

async function migrate() {
    console.log("🚀 Starting AI Registry Migration...");
        ...
        try {
            ...
            if (cap.tools && cap.tools.length > 0) {
                const toolsToInsert = cap.tools.map(t => ({
                    ...
                    requiredPermission: (TOOL_PERMISSION_MAP as any)[t.name] || "PLATFORM_ADMIN",
