...
export default function HospitalityPage() {
  const { data = [] } = useQuery({ queryKey: ['/api/hospitality-default'] });

  return (
    <div className="space-y-6 p-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">Hospitality & Travel</h1>
          ...
          <Card key={item.id || idx} className="hover-elevate" data-testid={`card-item-${item.id || idx}`}>
            <CardHeader className="pb-3">
              <div className="flex justify-between items-start">
                <CardTitle>{item.name || item.propertyName || item.guestName || item.id}</CardTitle>
            ...
            <CardContent className="grid grid-cols-3 gap-4 text-sm">
              ...
              <p><span className="text-muted-foreground">Value:</span> {item.revenue || item.rate || item.guests || "—"}</p>
            </CardContent>
          </Card>
        ))}
      </div>
