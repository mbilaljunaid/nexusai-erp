...
export default function AccountsPayable() {
    ...
    const handleSeedData = async () => {
        try {
            ...
            const s = await api.ap.suppliers.create({ name: "Acme Corp", taxId: "US-12345", country: "USA", riskScore: "10" });
            // Create a test invoice
            await api.ap.invoices.create({ supplierId: s.id, invoiceNumber: "INV-" + Date.now(), amount: "5000.00", currency: "USD", dueDate: new Date() });
        ...
        <div className="space-y-6 relative min-h-screen pb-20 p-6">
            ...
            <Tabs defaultValue="invoices" value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                <div className="flex items-center justify-between">
                    <TabsList className="bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 p-1 border shadow-sm rounded-lg lg:w-[400px]">
