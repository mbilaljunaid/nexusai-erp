  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard title="Total Revenue" value={`$${invoices.reduce((sum, inv) => sum + Number(inv.amount || 0), 0).toLocaleString()}`} change={metrics && 'growthRate' in metrics ? (metrics as any).growthRate : 8.2} icon={DollarSign} />
        <MetricCard title="Conversion Rate" value={`${metrics && 'conversionRate' in metrics ? (metrics as any).conversionRate : 24.8}%`} change={-2.1} icon={Target} />
        <MetricCard title="Forecast Accuracy" value={`${forecast && 'confidence' in forecast ? (forecast as any).confidence : 95}%`} change={5} icon={FolderKanban} />
      </div>
