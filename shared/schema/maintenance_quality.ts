...
export const maintInspections = pgTable("maint_inspections", {
    id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
    definitionId: varchar("definition_id").references(() => maintInspectionDefinitions.id).notNull(),

    workOrderId: varchar("work_order_id").references(() => maintWorkOrders.id),
    assetId: varchar("asset_id").references(() => faAssets.id),
    ...
    conductedByUserId: varchar("conducted_by_user_id"), // Ideally FK to users, but focusing on Schema independence
...
export const maintPermits = pgTable("maint_permits", {
    ...
    authorizedByUserId: varchar("authorized_by_user_id"),
