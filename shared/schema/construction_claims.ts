...
export const constructionClaims = pgTable("construction_claims", {
    ...
    contractId: varchar("contract_id").references(() => constructionContracts.id).notNull(),
    variationId: varchar("variation_id").references(() => constructionVariations.id), // Link to a cost variation if applicable
    ...
    reportedBy: varchar("reported_by"), // Reference to user/resource
