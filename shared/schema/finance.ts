import { pgTable, varchar, text, timestamp, numeric, boolean, integer, serial, jsonb, index, uniqueIndex } from "drizzle-orm/pg-core";
...
export const glRecurringJournals = pgTable("gl_recurring_journals", {
    ...
    recurringBatchName: varchar("recurring_batch_name").notNull(),
    description: text("description"),

    // Schedule
    frequency: varchar("frequency").default("Monthly"), // Monthly, Weekly, Quarterly
    status: varchar("status").default("Active"), // Active, Inactive, Completed
    ...
    templateJournalId: varchar("template_journal_id"), // FK to glJournals which acts as template
...
export const glBalances = pgTable("gl_balances_v2", {
    ...
    return {
        ...
        ledgerPeriodIdx: index("gl_balances_ledger_period_idx").on(table.ledgerId, table.periodName),
    };
});
...
export const glPeriodCloseChecklistTemplates = pgTable("gl_period_close_checklist_templates", {
    ...
    dayOffset: integer("day_offset").default(0), // T-Minus days (e.g. -2)
    createdAt: timestamp("created_at").default(sql`now()`),
});
...
export const glAutoPostRules = pgTable("gl_auto_post_rules", {
    ...
    effectiveDateFrom: timestamp("effective_date_from"),
    priority: integer("priority").default(10), // Added priority
    enabled: boolean("enabled").default(true),
    createdAt: timestamp("created_at").default(sql`now()`),
});
...
export const glApprovalRules = pgTable("gl_approval_rules", {
    id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
    name: varchar("name").notNull(),
    ledgerId: varchar("ledger_id").notNull(),
    ...
    enabled: boolean("enabled").default(true),
    createdAt: timestamp("created_at").default(sql`now()`),
    ruleName: varchar("rule_name"), // Legacy column restoration
});
export const insertGlApprovalRuleSchema = createInsertSchema(glApprovalRules);
...
export const glApprovalHistory = pgTable("gl_approval_history", {
    id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
    journalId: varchar("journal_id").notNull(),
    action: varchar("action").notNull(), // SUBMIT, APPROVE, REJECT
    actorId: varchar("actor_id").notNull(),
    comments: text("comments"),
    actionDate: timestamp("action_date").default(sql`now()`),
});
...
export const glConsolidationRuns = pgTable("gl_consolidation_runs", {
    id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
    ledgerSetId: varchar("ledger_set_id").notNull(), // The group being consolidated
    periodId: varchar("period_id").notNull(),
    status: varchar("status").default("Pending"), // Pending, Running, Completed, Error
    runDate: timestamp("run_date").default(sql`now()`),
    completedDate: timestamp("completed_date"),
    totalEliminations: numeric("total_eliminations", { precision: 18, scale: 2 }).default("0"),
    errorLog: text("error_log"), // Detailed run log
});
export const insertGlConsolidationRunSchema = createInsertSchema(glConsolidationRuns);
...
export const glEliminationDefinitions = pgTable("gl_elimination_definitions", {
    id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
    name: varchar("name").notNull(),
    description: text("description"),
    ledgerSetId: varchar("ledger_set_id"), // Optional scope

    // Match Criteria
    matchRule: varchar("match_rule"),   // e.g. "Segment3=4000" (Intercompany Payables)

    // Elimination Logic
    eliminationLedgerId: varchar("elimination_ledger_id"), // Where to post the elimination entry
    thresholdAmount: numeric("threshold_amount", { precision: 18, scale: 2 }), // Minimum amount to process

    enabled: boolean("enabled").default(true),
    createdAt: timestamp("created_at").default(sql`now()`),
});
export const insertGlEliminationDefinitionSchema = createInsertSchema(glEliminationDefinitions);
