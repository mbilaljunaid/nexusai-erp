...
export const constructionDailyEquipment = pgTable("construction_daily_equipment", {
    id: uuid("id").primaryKey().defaultRandom(),
    dailyLogId: uuid("daily_log_id").notNull(),
    equipmentType: text("equipment_type").notNull(), // e.g., Excavator, Crane
    equipmentId: text("equipment_id"), // Optional Asset ID
    hoursUsed: decimal("hours_used", { precision: 10, scale: 2 }).notNull(),
    workPerformed: text("work_performed"),
    costStatus: text("cost_status").default("UNCOSTED"), // UNCOSTED, COSTED
});
...
export const insertDailyEquipmentSchema = createInsertSchema(constructionDailyEquipment);
