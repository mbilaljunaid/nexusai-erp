...
import { pgTable, text, serial, integer, boolean, timestamp, jsonb, numeric, varchar, date, uuid } from "drizzle-orm/pg-core";
...
export const lcmCostComponents = pgTable("lcm_cost_components", {
    ...
    absorptionAccountCcid: varchar("absorption_account_ccid"), // Credit Account for Estimates
    varianceAccountCcid: varchar("variance_account_ccid"), // Dr/Cr Variance Account
...
export const lcmTradeOperations = pgTable("lcm_trade_operations", {
    ...
    approvalStatus: varchar("approval_status", { length: 20 }).default('DRAFT'), // DRAFT, PENDING, APPROVED, REJECTED
...
export const lcmAllocations = pgTable("lcm_allocations", {
    ...
    varianceAmount: numeric("variance_amount"), // The difference between Estimated and Actual allocation
    createdAt: timestamp("created_at").default(sql`now()`),
});

export const lcmAuditLogs = pgTable("lcm_audit_logs", {
    id: uuid("id").primaryKey().defaultRandom(),
    entityTable: varchar("entity_table").notNull(), // 'lcm_trade_operations', 'lcm_charges', 'lcm_allocations'
    entityId: varchar("entity_id").notNull(),
    action: varchar("action").notNull(), // 'CREATE', 'UPDATE', 'DELETE', 'ALLOCATE', 'CLOSE'
    changedFields: jsonb("changed_fields"), // { old: ..., new: ... }
    performedBy: varchar("performed_by").default('SYSTEM'), // User ID or 'SYSTEM'
    createdAt: timestamp("created_at").default(sql`now()`),
});

export const lcmAuditLogRelations = relations(lcmAuditLogs, ({ one }) => ({
