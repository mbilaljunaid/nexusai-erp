...
// Epics
export const epicRecords = pgTable("epic_records", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  epicKey: varchar("epic_key").notNull().unique(),
  name: varchar("name").notNull(),
  description: text("description"),
  status: varchar("status").default("backlog"), // backlog, active, closed
...
export const insertEpicSchema = createInsertSchema(epicRecords).omit({ id: true, createdAt: true, updatedAt: true }).extend({
  epicKey: z.string().min(1),
  name: z.string().min(1),
  description: z.string().optional().nullable(),
  status: z.enum(["backlog", "active", "closed"]).optional(),
...
export type Epic = typeof epicRecords.$inferSelect;

// Stories
export const storyRecords = pgTable("story_records", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  storyKey: varchar("story_key").notNull().unique(),
...
export const insertStorySchema = createInsertSchema(storyRecords).omit({ id: true, createdAt: true, updatedAt: true }).extend({
...
export type Story = typeof storyRecords.$inferSelect;

// Project Tasks
export const projectTasks = pgTable("project_tasks", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  taskKey: varchar("task_key").notNull().unique(),
  storyId: varchar("story_id").notNull(),
  title: varchar("title").notNull(),
  description: text("description"),
  status: varchar("status").default("todo"), // todo, in_progress, done
  assignee: varchar("assignee"),
...
export const insertProjectTaskSchema = createInsertSchema(projectTasks).omit({ id: true, createdAt: true, updatedAt: true }).extend({
...
// Sprints
export const sprintRecords = pgTable("sprint_records", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  sprintKey: varchar("sprint_key").notNull().unique(),
  name: varchar("name").notNull(),
  status: varchar("status").default("planning"), // planning, active, completed, closed
  startDate: timestamp("start_date"),
  endDate: timestamp("end_date"),
  goal: text("goal"),
...
export const insertSprintSchema = createInsertSchema(sprintRecords).omit({ id: true, createdAt: true, updatedAt: true }).extend({
  sprintKey: z.string().min(1),
  name: z.string().min(1),
...
export type InsertSprint = z.infer<typeof insertSprintSchema>;
export type Sprint = typeof sprintRecords.$inferSelect;
...
export const kanbanBoards = pgTable("kanban_boards", {
  ...
  name: varchar("name").notNull(),
  teamId: varchar("team_id"),
  columns: jsonb("columns"), // ["Todo", "In Progress", "Review", "Done"]
  createdAt: timestamp("created_at").default(sql`now()`),
  updatedAt: timestamp("updated_at").default(sql`now()`),
});

export const insertKanbanBoardSchema = createInsertSchema(kanbanBoards).omit({ id: true, createdAt: true, updatedAt: true }).extend({
  name: z.string().min(1),
  teamId: z.string().optional().nullable(),
...
export const timeLogs = pgTable("time_logs", {
  ...
  userId: varchar("user_id").notNull(),
  hoursLogged: numeric("hours_logged", { precision: 5, scale: 2 }).notNull(),
  logDate: timestamp("log_date").default(sql`now()`),
  description: text("description"),
  createdAt: timestamp("created_at").default(sql`now()`),
});

export const insertTimeLogSchema = createInsertSchema(timeLogs).omit({ id: true, createdAt: true, logDate: true }).extend({
  taskId: z.string().min(1),
  userId: z.string().min(1),
  hoursLogged: z.string().min(1),
  description: z.string().optional().nullable(),
});
...
export const burndownRecords = pgTable("burndown_records", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  sprintId: varchar("sprint_id").notNull(),
  day: integer("day").notNull(), // day of sprint (1-14)
...
export const insertBurndownDataSchema = createInsertSchema(burndownRecords).omit({ id: true, recordedAt: true }).extend({
...
export type InsertBurndownData = z.infer<typeof insertBurndownDataSchema>;
export type BurndownData = typeof burndownRecords.$inferSelect;

// Team Members (for project assignment)
export const projectTeams = pgTable("project_teams", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  teamName: varchar("team_name").notNull(),
  description: text("description"),
  members: text("members"), // JSON stringified user IDs
  createdAt: timestamp("created_at").default(sql`now()`),
});

export const insertProjectTeamSchema = createInsertSchema(projectTeams).omit({ id: true, createdAt: true }).extend({
