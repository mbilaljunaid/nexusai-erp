...
export default function AIChat() {
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async (content: string) => {
      ...
      // 2. Call AI Service Intent Parser
      ...
      const aiAction = await resp.json();

      // 3. Add AI Response (Action Schema)
      const aiResponseContent = `**Intent:** ${aiAction.intent}\n**Action:** \`${aiAction.action}\`\n**Domain:** ${aiAction.domain}\n**Confidence:** ${aiAction.confidence}`;

      setMessages(prev => [...prev, { role: "assistant", content: aiResponseContent, raw: aiAction }]);
            ...
            <div className="space-y-1 max-h-96 overflow-y-auto">
              {conversations.map((conv: any) => (
                <div
                  ...
                  className={`p-2 rounded cursor-pointer text-sm truncate ${activeConvId === conv.id ? "bg-primary text-primary-foreground" : "hover:bg-accent"
                    }`}
            ...
            <div className="flex-1 space-y-3 overflow-y-auto">
              {messages.map((msg: any, idx: number) => (
                <div key={idx} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div
                    className={`max-w-xs px-4 py-2 rounded-lg ${msg.role === "user" ? "bg-primary text-primary-foreground" : "bg-accent text-accent-foreground"
                      }`}
